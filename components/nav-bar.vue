<template>
  <header class="fixed top-0 left-0 w-full z-50  px-6 py-4 flex justify-between items-center">
    ...
    <nav class="hidden md:flex gap-6 text-xl text-yellow-300">
      ...
      <NuxtLink to="/events" class="hover:text-red-500">Events</NuxtLink>
      <NuxtLink to="/vintage" class="hover:text-red-500">Vintage</NuxtLink>
      <NuxtLink to="/about" class="hover:text-red-500">About</NuxtLink>
      <NuxtLink to="/store" class="hover:text-red-500">Store</NuxtLink>
    </nav>
    <div >
        <UButton color="secondary" variant="soft">Login</UButton>
    </div>
  </header>
</template>
